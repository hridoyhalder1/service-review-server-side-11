...
async function run(){
    try{
        ...
        const serviceCollectionn = client.db('servicesUser').collection('allServices');
        ...
        // allServices api create
        app.get('/allServices', async(req, res) => {
            const query = {}
            const cursor = serviceCollectionn.find(query);
...
app.get('/', (req, res) => {
    res.send('services server site running');
})

app.listen(port, () => {
    console.log(`services server site running on ${port}`);
})